...
class TestMinIOStorageIntegration:
    ...
    @pytest.mark.asyncio
    async def test_upload_pdf_creates_file(self, minio_storage: IFileStorage):
        # Arrange
        ...
        # Act
        await minio_storage.upload(filename, content, content_type)

        # Assert
        assert await minio_storage.exists(
            filename
        ), f"Arquivo {filename} deveria existir após upload"

    @pytest.mark.asyncio
    async def test_upload_empty_file(self, minio_storage: IFileStorage):
        # Arrange
        filename = "empty.txt"
    ...
    @pytest.mark.asyncio
    async def test_upload_with_none_content_type(self, minio_storage: IFileStorage):
        # Arrange
        filename = "binary.bin"
    ...
    async def test_exists_returns_true_for_existing_file(
        ...
        await minio_storage.upload(filename, content, "text/plain")

        # Act
        result = await minio_storage.exists(filename)
    ...
    @pytest.mark.asyncio
    async def test_download_retrieves_uploaded_content(
        self, minio_storage: IFileStorage
    ):
        # Arrange
        filename = "content_test.txt"
        original_content = b"This is the original content that was uploaded"
        await minio_storage.upload(filename, original_content, "text/plain")

        # Act
        downloaded_content = await minio_storage.download(filename)
    ...
    @pytest.mark.asyncio
    async def test_download_empty_file(self, minio_storage: IFileStorage):
        # Arrange
    ...
    async def test_download_nonexistent_file_raises_error(
        ...
        with pytest.raises(Exception):
            await minio_storage.download(filename)

    @pytest.mark.asyncio
    async def test_multiple_uploads_same_path_overwrites(
        self, minio_storage: IFileStorage
    ):
        # Arrange
        ...
        # Act - Upload versão 1
        await minio_storage.upload(filename, content_v1, "text/plain")
        assert await minio_storage.download(filename) == content_v1

        # Act - Upload versão 2 (sobrescreve)
        await minio_storage.upload(filename, content_v2, "text/plain")

        # Assert - Versão 2 está lá
        assert (
            await minio_storage.download(filename) == content_v2
