...
@router.post("/")
async def create_session(dto: MessageDTO, session_service: SessionService=Depends(get_session_service)) -> SessionDTO:
    return SessionMapper.to_dto(await session_service.create_session(MessageMapper.to_entity(dto)))

@router.get("/{session_id}")
async def open_session(session_id: int, session_service: SessionService=Depends(get_session_service)) -> SessionDTO:
